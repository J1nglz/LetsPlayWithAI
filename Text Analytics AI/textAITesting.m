...
%% most probably topic words turn this into a function
...
% queryWiki = "https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=Space%20Shuttle&format=jsonfm";
% queryWiki = "https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=Space%20Shuttle&format=jsonfm";
% rawWiki = "https://en.wikipedia.org/w/index.php?action=raw&title=Space_Shuttle";
% test_json = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=revisions&list=search&continue=&rvprop=content%7Cids&srsearch=Space%20Shuttle&srsort=relevance";
% 
% wikiSpaceShuttleOpenSearch = "https://en.wikipedia.org/w/api.php?action=opensearch&format=json&search=Space%20Shuttle";
% wikiSpaceShuttleOpenSearch = webread(wikiSpaceShuttleOpenSearch);
% wikiContent = "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=revisions&continue=&pageids=28189&rvprop=content";
...
V_normWater=word2vec(embNorm,"water");
V_normFire=word2vec(embNorm,"fire");
vec2word(embNorm,V_normWater+V_normFire)
M = V_normFire+V_normWater;
k=5;
[words,dist] = vec2word(embNorm,M,k,'Distance','euclidean');
figure;
bar(dist)
xticklabels(words)
xlabel("Word")
