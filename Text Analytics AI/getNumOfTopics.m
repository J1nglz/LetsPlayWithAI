function [numOfTopics] = getNumOfTopics(documentsIn,percentHoldOut,numTopicsRange)
%UNTITLED6 Summary of this function goes here
%   Detailed explanation goes here

bag = bagOfWords(documentsIn);
bag = removeInfrequentWords(bag,2);
bag = removeEmptyDocuments(bag);
numDocuments = numel(documentsIn);
cvp = cvpartition(numDocuments,'HoldOut',percentHoldOut); %holdout 10% to test with
...
numOfTopics = numTopicsRange(find(validationPerplexity==min(validationPerplexity)));
